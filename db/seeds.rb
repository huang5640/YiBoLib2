...
book = Book.search_douban_by_isbn(isbn)
		  Book.create!([
								{title: book["title"], author: book["author"], description: book["summary"], ISBN: book["isbn13"], image: book["image"], YiBoNum: Faker::Number.number(16),location: Location.order("RANDOM()").first }
								])
end
